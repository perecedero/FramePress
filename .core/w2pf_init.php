	...
	set_error_handler('w2pf_eh');
		...
		if(!file_exists($temp . DS . 'press_tmp'))
		{
			...
			else
			{
				if( substr(sprintf('%o', fileperms(dirname(dirname(__FILE__)) . DS . 'tmp')), -3) < '777'){
					trigger_error("Can&#39;t write on <b>". dirname(dirname(__FILE__)) . DS . 'tmp'."</b> folder, please change it's permissions to 777", E_USER_ERROR);exit;
				}
			}
		}
		...
		*/
		if (class_exists($FramePress)) {
			trigger_error("Sorry prefix: <b>" . $config['prefix'] . "</b> is used by other plugin on this WP installation, please choose a unique one", E_USER_ERROR);exit;
		}
